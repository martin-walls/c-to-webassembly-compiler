...
Six of the programs were sourced from Clib, which contains many utility packages for C~\ccite{clib}.
...
For all my tests, I used GCC\footnote{GCC version 11.3.1, with the default options.} as the reference for correctness~\ccite{gcc}.
...
\Ccref{tab:unreachable procedure elimination data} shows an excerpt of the data I collected, measuring the file size reduction due to this optimisation.
...
\begin{table}[t]
  ...
  \begin{tabular}{$l^r^r^r}
    \toprule
    \rowstyle{\bfseries}Test program & Unoptimised size (bytes) & Optimised size (bytes) & Reduction (bytes) \\
    \midrule
    \Filename{trim.c} & 8821 & 3642 & 5179 \\
    \Filename{case.c} & 13218 & 8881 & 4337 \\
    \Filename{hexify.c} & 4912 & 4879 & 33 \\
    \Filename{wildcardcmp.c} & 7280 & 7248 & 32 \\
    \Filename{fibonacci.c} & 2612 & 2595 & 17 \\
    \Filename{strlen.c} & 648 & 648 & 0 \\
    \bottomrule
  \end{tabular}
...
One of the functions I used to evaluate this optimisation uses tail-recursion to compute the sum of the first $n$ integers~(see \ccref{lst:tail-recursive sum}).
...
\begin{figure}[t]
  ...
  \caption{
    Stack usage plot for calling \CInline{sum(500, 0)}, showing how tail-call optimisation transforms the stack size from~\bigo{n} to~\bigo{1}.
    % The effect of tail-call optimisation on the stack size of the tail-recursive sum program in \ccref{lst:tail-recursive sum}.
  % Stack usage for calling \CInline{sum(500, 0)} (see \ccref{lst:tail-recursive sum}), showing the effect of tail-call optimisation.
  }
...
\Ccref{fig:comparing stack usage for case.c} shows the size of the stack over the execution of the \Filename{case.c} program that converts a string to upper, lower, and camel case in turn.
...
The program in turn calls \CInline{case_upper()}, \CInline{case_lower()}, and \CInline{case_camel()} on a string, corresponding to the three distinct sections of the plot.
...
\begin{listing}[p]
  ...
  \end{sublisting}
  \par\smallskip
  \caption{Comparing the body code of \CInline{case_upper()} and \CInline{case_camel()}.\bigskip}
...
\begin{figure}[p]
  ...
  \caption{Comparing stack usage for \Filename{case.c}, showing the effect of my optimised stack allocation policy.}
...
\begin{figure}[p]
  ...
  \caption{Comparing stack usage for \Filename{fibonacci.c}, showing the effect of my optimised stack allocation policy.}
  \label{fig:comparing stack usage for fibonacci.c} % chktex 24
\end{figure}

The functions \CInline{case_upper()} and \CInline{case_lower()} each make repeated calls to the library functions \CInline{toupper()} and \CInline{tolower()}, which correspond to the many short spikes on the plot (one for each character in the string).
...
\Ccref{lst:caseupper and casecamel body code} compares the body of \CInline{case_upper()} with a section of the code for \CInline{case_camel()}.
Even in the short section shown, more temporary variables are created than in the entire body of \CInline{case_upper()}.
This causes the larger spike at the end of the stack size plot (\ccref{fig:comparing stack usage for case.c}).
