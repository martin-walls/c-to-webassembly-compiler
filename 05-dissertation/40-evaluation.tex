...
For unreachable procedure elimination, I evaluated the reduction in code size.
For tail-call optimisation and stack allocation optimisation, I evaluated the effectiveness at reducing memory usage.
...
The other standard library headers I implemented contained only a few functions, so the impact of this optimisation was much more limited. However, as mentioned above, if I implemented more of the standard library, I would see much more improvement.
...
One of the functions I used to evaluate this optimisation was the function in \ccref{lst:tail-recursive sum}, that uses tail-recursion to compute the sum of the first $n$ integers.
...
\begin{figure}[t]
  ...
  \caption{Comparing optimised against unoptimised stack memory usage to usage. The shorter the optimised bar, the greater the improvement (less memory is used).}%
...
The more temporary variables generated, the larger each stack frame will be in the unoptimised version, and hence the more scope for the compiler to find non-clashing variables to overlap.
Since temporary variables are generated locally for each instruction, the majority only have short-range dependencies.
...
The result of this is that as a function increases its number of operations, the number of temporary variables proportionally increases, and so too the scope for optimisation that the compiler is able to exploit.
...
We can use this to interpret which parts of the plot correspond to which parts of the source program.
