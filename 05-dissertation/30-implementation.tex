...
\Indent{1}\Dirname{program_config} & Compiler constants and runtime options data structures. \\\hline % chktex 44
...
\begin{figure}[!htb]
  ...
  \label{fig:lexing numbers fsm} % chktex 24
\end{figure}
...
\begin{figure}[!htb]
  ...
  \label{fig:lexing identifiers fsm} % chktex 24
\end{figure}
...
Microsoft's C Language Syntax Summary \ccite{c-language-grammar-microsoft} and C: A Reference Manual \ccite{c-reference-manual} were very useful references to ensure I captured the subtleties of C's syntax when writing my grammar. % chktex 13
...
A naive grammar for C (\ccref{lst:ambiguous if-else grammar}) contains an ambiguity around \CInline{if}/\CInline{else} statements \ccite[sec.~8.5.2]{c-reference-manual}.
...
For each of these, the compiler calls \RustInline{convert_statement_to_ir(stmt, program, context)}. % chktex 36
...
The following types are supported by the \gls{ir}, mirroring the types supported by the C language \ccite[ch.~5]{c-reference-manual}.
...
For example, before generating a \IrCode{t = a < b} instruction, the compiler checks whether \IrCode{a} and \IrCode{b} are comparable arithmetic types---if not, a compile error is thrown---and sets the type of \IrCode{t} to \IrType{I32}\footnote{\IrType{I32} is used to represent booleans.}.
This ensures that the \gls{ir} passed to the back end is type safe.

\subsection{The Relooper Algorithm}

The Relooper algorithm is described in \ccref{sec:prep:relooper}. In this section I describe my implementation.
...
By this I mean any label instruction that is not preceded by a branch, where control will flow to directly from the previous instruction.
This is removed by inserting a branch instruction before each label instruction, if one does not already exist, branching to that label.
...
For every function in the \gls{ir}, the instructions are soupified and then the algorithm described in \ccref{sec:prep:relooper algorithm} is used to create a Relooper block.
The \RustInline{IdGenerator} struct described above is used to assign each loop and multiple block a unique ID\@.
...
The back end generates a \RustInline{WasmModule} containing \RustInline{WasmInstruction}s, the byte representation of which is written out to a binary file as output of the compiler.

I defined a \RustInline{ToBytes} trait that is implemented by all data structures that will be written out to the binary.
...
\begin{listing}[h]
  ...
  \end{subfigure}
  \caption{Transforming an add instruction to target code, assuming \IrCode{a} and \IrCode{b} are variables of type \IrType{i64}.}
...
I chose to use Node.js \ccite{nodejs}, because this allows the binary to be run locally.
...
The challenge of this optimisation was to find a (near) optimal way of allocating variables such that as little memory is used as possible.

\begin{mrwComment}
- remove dead variables

-> necessary for LVA to work properly

- generate clash graph

-> generate instruction flowgraph

-> live variable analysis

-> any vars live at same time clash

- FIFI queue of smallest least clashed variables

- allocate in turn from queue
\end{mrwComment}
