...
After processing \CInline{#include} directives, the compiler runs \mintinline{bash}{cpp} on the source code.
...
This ambiguity is impossible to resolve with the language grammar. The solution is to process \CInline{typedef} names during the lexing stage, and emit distinct type name and identifier tokens to the parser.
...
The lexer is implemented as a \gls{fsm}. \Ccref{fig:lexer fsm} highlights portions of the machine; the remaining state transition diagrams can be found in \ccref{app:lexer fsm}.
...
The handling of escape sequences in strings is described in~\ccref{app:sec:parsing string escape sequences}.
Subtleties around parsing type specifiers are discussed in~\ccref{app:sec:parsing type specifiers}.
...
After soupifying, \ccref{alg:relooper} (\ccref{sec:prep:relooper algorithm}) is used to create a Relooper block.
...
The instruction flowgraph is similar to the call graph used in \ccref{sec:impl:unreachable procedure elimination}, but at the level of individual instructions.
