...
\Ccref{lst:lva implementation} shows my implementation.
...
\begin{listing}[ht]
  ...
  \For {instruction $n$}
    ...
    \While {$|\ell| > 1$}
      ...
      \For {$\mathit{var}_2 \in \ell$}
        \State Add edge $(v_1, v_2)$ to clash graph
      \EndFor
    \EndWhile
    \If {$n$ takes the address of its operand $\mathit{var}$}
...
\begin{mrwComment}
- FIFI queue of smallest least clashed vars

- allocate in turn from queue

- prefer smaller addrs
\end{mrwComment}
