...
\begin{EnumerateAlgorithm}
\item Find all \IrCode{call} instructions that are directly returned from the function, that are recursive calls.
\item For each argument to the recursive call, copy it to the variable holding the corresponding parameter in the caller's stack frame.
\item Insert a \IrCode{br} instruction back to the start of the function.
\end{EnumerateAlgorithm}
