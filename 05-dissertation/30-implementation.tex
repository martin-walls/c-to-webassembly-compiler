...
My grammar is able to parse all core features of the C language, whilst omitting some of the recent additions. I chose to make my parser handle a larger subset of C than the compiler as a whole supports; the middle end rejects or ignores nodes of the \gls{ast} that it does not handle. For example, the parser handles storage class specifiers (e.g.~\CInline{static}) and type qualifiers (e.g.~\CInline{const}), and the middle end simply ignores them.
...
LALRPOP allows macros to be defined, which allow the grammar to be written in a more intuitive way.
...
\RustInline{context} is the context object described in \ccref{app:sec:context object design pattern}, which passes relevant contextual information through the functions recursively.
