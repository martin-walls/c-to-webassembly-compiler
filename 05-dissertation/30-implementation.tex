...
The macro has a generic type \RustInline{T}, and collects the list items into a \RustInline{Vec<T>}, which can be used by the rules that use the macro.
...
\begin{listing}[!ht]
  \begin{RustListing}
    ...
    struct IdGenerator<T: Id + Clone> {
        max_id: Option<T>,         // Internally tract the highest ID used so far
    }
  ...
  \end{RustListing}
  \caption{Implementation of the \RustInline{Id} trait and \RustInline{IdGenerator} struct.}
...
\begin{listing}[!ht]
  ...
  \end{RustListing}
  \caption{Generating \gls{ir} code for a \CInline{while} statement.}
...
The Relooper algorithm is described in \ccref{sec:prep:relooper} \textcolor{red}{\textit{[link to Preparation chapter: Relooper section]}}. In this section I describe my implementation.
...
For every function in the \gls{ir}, the instructions are soupified and then the algorithm described in \ccref{sec:prep:relooper algorithm} \textcolor{red}{\textit{[link to Preparation chapter: Relooper algorithm section]}} is used to create a Relooper block.
