...
In today's WebAssembly compilers, there are three general methods used to convert to structured control flow: Emscripten's Relooper algorithm, LLVM's CFGStackify, and Cheerp's Stackifier~\ccite{wasm-control-flow-restructuring-thread,solving-structured-control-flow-problem}.
...
The Emscripten paper outlines a proof that this greedy approach will always succeed~\ccite{emscripten}.
...
When the entire pipeline was complete, I used GCC as a reference compiler; I compiled my test programs with both my compiler and GCC, and verified that the resulting binaries produced the same output~\ccite{gcc}.
...
Additional development scripts, for automated testing and profiling, were written in Python, because of the ease of use, and my familiarity with libraries such as Matplotlib~\ccite{python,matplotlib}.
...
I used Git for version control with GitHub as an off-site backup~\ccite{git,github}, and the CLion IDE as my development environment using the Intellij Rust plugin~\ccite{clion,intellij-rust-plugin}.
