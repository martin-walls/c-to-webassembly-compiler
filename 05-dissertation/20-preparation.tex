...
Initially, I extensively researched the WebAssembly specification~\ccite{wasm-spec} to gain a deep understanding of the target binary code.
...
In today's WebAssembly compilers, there are three general methods used to convert to structured control flow~\ccite{wasm-control-flow-restructuring-thread, solving-structured-control-flow-problem}: Emscripten's Relooper algorithm, LLVM's CFGStackify, and Cheerp's Stackifier.
...
Given a set of labels, and set of entries, \ccref{alg:relooper} creates a block:
...
I learnt the Rust programming language~\ccite{rust-lang} for this project.
...
I made use of Rust's excellent online documentation~\ccite{rust-by-example, rust-book} to become familiar with the language.
...
I used Git~\ccite{git} as my version control system.
I regularly committed my code in small, revertible chunks, and pushed the repository to GitHub~\ccite{github} to maintain an up-to-date off-site backup.
...
When the entire pipeline was complete, I used GCC~\ccite{gcc} as a reference compiler; I compiled my test programs with both my compiler and GCC, and verified that the resulting binaries produced the same output.
...
I used Git~\ccite{git} for version control and GitHub~\ccite{github} as an off-site backup.
